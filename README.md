...
- **Foto pemain** — foto asli dari `play.fifa.com/media/image/headshots/{fifaId}_headshot.png` (CDN publik FIFA, CORS terbuka), pakai field `fifaId` yang sudah ada di `players.json`. Logic-nya di [player-photo.js](fifa-fantasy/player-photo.js): kalau pemain tidak punya `fifaId`, atau gambar gagal dimuat (404/403 — sekitar 240 dari 1488 pemain belum punya foto di sumbernya), otomatis fallback ke avatar inisial (lingkaran warna deterministik dari nama). Dipakai di semua tempat pemain ditampilkan: modal detail, roster tim, tabel daftar pemain, player compare, susun skuad, dan dashboard.
